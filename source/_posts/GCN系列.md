...
网络学习的重点在于找到那些**适合匹配的特征点**，而不是普通意义上的那些**high gradient points**。作者先在一幅图像中提取high gradient points，然后通过warping找到这些点在另一幅图像中的对应点。事实上这些warp到新图像中的点也许并不是该图像上的high gradient points。
...
其中的f为CNN输出的feature map，![](/all_images/GCN系列/4.png)是两个feature map的L2距离，m为margin，在实验中被设定为1，x为第一幅图像中的特征点，y\*为正样本，是x点warp到第二幅图像中的点，z\*为负样本，是第二幅图像中除了y以外所有的特征点中与x点的feature map的L2距离最小的点，这个样本的选取策略称为**hardest negative sample mining**。其实就是正样本以外的与anchor距离最近的样本，这个loss的设计有点**类似特征匹配后的radio test**，都是希望最好的匹配与次好的匹配之间存在一定的差距。
<div align="center">
...
其中o为预测的关键点，cx为2D点的标签(提前在数据集上提取Harris角点得到标签)，实验中![](/all_images/GCN系列/7.png)和![](/all_images/GCN系列/8.png)分别为0.1和1.0
...
### 二值化描述子
GCNv2设计了和ORB同样格式的二进制描述子，在网络的top层增加一个binary激活层(但是作者在**github中给出的网络结构的源码中并没有这一层**)，这层是被设计为一符号函数的形式，因此并不是处处都可导的，作者采用了[Binarized Neural Networks](https://arxiv.org/pdf/1602.02830.pdf)中的解决方法，最终二进制激活层的形式如下
<div align="center">
...
其中m是margin(在实验中被设定为1)，d(x,y)为x,y两个向量的**汉明距离的平方**，作者称使用平方是因为这样训练更容易收敛。![](/all_images/GCN系列/18.png)为正确的匹配对，类似GCNv1，正样本是通过groundtruth的位姿通过warp得到的
<div align="center">
...
## GCN-SLAM
作者在ORB-SLAM2中使用GCNv2代替ORB特征，名为GCN-SLAM，作者称在Intel i7-7700HQ和mobile version NVIDIA 1070的笔记本上可以达到70fps并在TX2上达到20fps，GCNv2-tiny可以达到40fps。特征提取的流程对比如下
<div align="center">
<img src="/all_images/GCN系列/22.png">
</div>

具体对ORB-SLAM2的改动：
* 用GCNv2代替ORB并采用一种新的非极大值抵制的方法

* 关掉匀速运动模型，只采用参考帧模型，并且在参考中模型中使用最近邻匹配的方法进行匹配

* 关掉闭环检测以及位姿图优化

* 在训练所用的数据集上重新生成词袋

其中第二点和第三点都是为了排除其他因素对特征点性能评估造成的影响。

## 实验
作者在SUN3D数据集上按1fps的频率进行采样，共得到44624张图像用于训练。在TUM数据集上进行评估，结果如下
<div align="center">
